...
class JobStore:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            ...
            migrations = {
                "worker_backend": "ALTER TABLE jobs ADD COLUMN worker_backend TEXT",
                ...
                "next_retry_at": "ALTER TABLE jobs ADD COLUMN next_retry_at TEXT",
                "last_error_at": "ALTER TABLE jobs ADD COLUMN last_error_at TEXT",
                "webhook_attempts": "ALTER TABLE jobs ADD COLUMN webhook_attempts INTEGER DEFAULT 0",
                "webhook_last_error": "ALTER TABLE jobs ADD COLUMN webhook_last_error TEXT",
                "webhook_last_status_code": "ALTER TABLE jobs ADD COLUMN webhook_last_status_code INTEGER",
                "webhook_delivery_id": "ALTER TABLE jobs ADD COLUMN webhook_delivery_id TEXT",
                "webhook_delivered_at": "ALTER TABLE jobs ADD COLUMN webhook_delivered_at TEXT",
            }
    ...
    def upsert_job(self, job: dict[str, Any]) -> None:
        with self._lock, self._connect() as connection:
            connection.execute(
                """
                INSERT INTO jobs (
                    job_id, status, owner, created_at, completed_at, output_path, result_id,
                    summary_json, error, config_json, webhook_url, webhook_status, webhook_attempted_at
                    , webhook_attempts, webhook_last_error, webhook_last_status_code, webhook_delivery_id, webhook_delivered_at
                    , worker_backend, locked_by, lease_until, started_at, attempts, next_retry_at, last_error_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    ...
                    job.get("output_path"),
                    job.get("result_id"),
                    json.dumps(job.get("summary"), ensure_ascii=False) if job.get("summary") is not None else None,
                    job.get("error"),
    ...
    def claim_next_job(self, worker_id: str, lease_seconds: float = 7200.0) -> dict[str, Any] | None:
    ...
    def claim_job(self, job_id: str, worker_id: str, lease_seconds: float = 7200.0) -> dict[str, Any] | None:
        now = datetime.now(UTC)
        lease_until = now + timedelta(seconds=max(1.0, float(lease_seconds)))
        now_text = now.isoformat()
        lease_text = lease_until.isoformat()
    ...
    def renew_lease(self, job_id: str, worker_id: str, lease_seconds: float = 7200.0) -> dict[str, Any] | None:
        now = datetime.now(UTC)
        lease_until = now + timedelta(seconds=max(1.0, float(lease_seconds)))
        with self._lock, self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM jobs WHERE job_id = ? AND status = 'running' AND locked_by = ?",
                (job_id, worker_id),
            ).fetchone()
            if not row:
                return None
        ...
        *,
        ...
        with self._lock, self._connect() as connection:
            ...
            if retryable and attempts < max_attempts:
                next_status = "queued"
                next_retry_at = (datetime.now(UTC) + timedelta(seconds=max(0.0, float(retry_delay_seconds)))).isoformat()
            ...
            connection.execute(
                ...
                WHERE job_id = ?
                """,
                (next_status, error, completed_at, locked_by, lease_until, next_retry_at, last_error_at, job_id),
            )
            connection.commit()
            updated = connection.execute("SELECT * FROM jobs WHERE job_id = ?", (job_id,)).fetchone()
        return self._row_to_job(updated) if updated else None
